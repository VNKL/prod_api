...
class VKAuth(object):
    ...
    def __init__(self, permissions, app_id, api_v, email=None, pswd=None, two_factor_auth=False, security_code=None,
        ...
        """
        self.proxy_dict = {'http': f'http://{proxy}'} if proxy else None
    ...
    def auth(self):
        ...
        self.response = self.session.get(auth_url, proxies=self.proxy_dict)
    ...
    def _submit_form(self, *params):
        ...
        if parser.method == 'post':
            ...
            try:
                self.response = self.session.post(parser.url, data=payload, proxies=self.proxy_dict)
